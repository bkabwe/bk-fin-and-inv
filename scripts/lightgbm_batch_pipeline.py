...
class GitHubReleaseClient:
    ...
    def upload_asset(self, release: dict[str, Any], asset_path: Path, asset_name: str) -> dict[str, Any]:
        ...
        for attempt in range(1, 4):
            logger.info("Uploading asset %s attempt %d/3", asset_name, attempt)
            try:
                ...
                logger.warning("Upload failed for %s on attempt %d/3: %s", asset_name, attempt, exc)
            ...
            if response.status_code >= 400:
                ...
                logger.warning(
                    "Upload failed for %s on attempt %d/3: HTTP %s",
                    asset_name,
                    attempt,
...
def discover(args: argparse.Namespace) -> int:
    ...
    if max_tickers > 0 and len(filtered) > max_tickers:
        ...
        duplicate_rows: list[tuple[int, dict[str, Any]]] = []
        ...
        for original_index, item in ranked:
            ...
            if ticker in seen_tickers:
                duplicate_rows.append((original_index, item))
                continue
...
def _promote_outputs(args: argparse.Namespace, batch_path: Path, manifest_path: Path, release_tag: str) -> None:
    ...
    logger.info("Ensuring batch release exists for tag=%s", release_tag)
    ...
    logger.info("Uploading merged batch asset path=%s", batch_path)
    client.upload_asset(batch_release, batch_path, DEFAULT_BATCH_ASSET_NAME)
    logger.info("Ensuring live manifest release exists for tag=%s", DEFAULT_LIVE_RELEASE_TAG)
    ...
    logger.info("Uploading live manifest asset path=%s", manifest_path)
...
def reduce_and_validate(args: argparse.Namespace) -> int:
    ...
    if args.macro_file:
        macro_rows = len(macro_table) if macro_table is not None else 0
        logger.info("Loaded shared macro table from %s with %d rows", args.macro_file, macro_rows)
    repository = resolve_release_repository(args.repository)
    created_at = _utc_now().isoformat().replace("+00:00", "Z")
    batch_id = str(args.batch_id).strip()
    logger.info("Merging partial batches for batch_id=%s", batch_id)
    current_models, current_meta, expected_tickers, extras = _merge_partial_batches(partial_files)
    logger.info("Merged partials into %d current models", len(current_models))
    logger.info("Validating expected horizons and training row counts")
    ...
    logger.info("Creating and persisting current merged batch to %s", args.output_batch)
    ...
    logger.info("Validating merged batch reload from %s", merged_batch_path)
    ...
    logger.info("Running smoke test predictions on sanity tickers=%s", tuple(args.sanity_tickers))
    _smoke_test_predictions(current_models, macro_table, tuple(args.sanity_tickers))

    logger.info("Loading previous live manifest and batch state from %s", repository)
    previous_manifest, previous_batch = _load_previous_live_state(repository)
    logger.info("Merging current batch with previous live state")
    ...
    logger.info("Building live manifest and writing to %s", args.output_manifest)
    ...
    merged_summary = (
        f"Merged current={len(current_models)} live={len(live_models)} "
        f"carried_forward={len(carried_forward)} dropped_stale={len(dropped_stale)}"
    )
    logger.info(merged_summary)
    print(merged_summary, flush=True)
    if args.promote:
        logger.info("Promote flag enabled; uploading batch and manifest to releases")
        _promote_outputs(args, merged_batch_path, manifest_path, release_tag)
        logger.info("Promoted release tag %s", release_tag)
        print(f"Promoted release tag {release_tag}", flush=True)
    return 0
