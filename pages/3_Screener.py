...
if not is_polygon_configured():
    st.warning("POLYGON_API_KEY is not configured. Configure it to enable scanning with Polygon data.")
...
if st.button("Run Screener"):
    ...
    if map_universe[label] in ("nasdaq", "otc", "nyseamerican"):
        ...
        if count and count < 50:
            st.warning(f"⚠️ {label} ticker fetch returned only {count} tickers. Results may be incomplete.")
    if results.empty:
